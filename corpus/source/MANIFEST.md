...
scans are gitignored (`raw/ideonomy.mit.edu/`, `raw/uh-mirror/*.pdf`) and
archived off-repo; the pre-fold staging history is likewise archived.
...
| Artifact | Contents | Status |
|---|---|---|
| `raw/wayback/pages/` | 269 HTML pages (all 301 tier-1 URLs, 0 failures): `/divisions/`, `/essays/`, `/investigable/`, `/analogy/`, `/maps/`, `/whatcan*/`, `/slides/`, top-level | acquired; **extracted (staging)** — 29 lists / 2,045 items in `extracted/wayback/` |
...
1. Wayback HTML → structured lists: **repo-grade done** (19 lists / 1,931
   items in the repo's `canon-wayback.jsonl`; earlier staging pass kept in
   `extracted/wayback/`). Remaining: non-`<li>` matter (DL glossaries,
   prose-embedded lists).
2. 403 chart photos → vision extraction: **done 2026-09-03** — 490 lists /
   ...
   legibility labels, rail recorded per record.
3. Monographs (6 + 16 PDFs) → OCR: **done 2026-09-03**, 3,266/3,266 pages
   (`extracted/ocr/`, tesseract 300dpi; 163 near-blank chart pages flagged
   for the vision rail). LLM structuring into canon lists: pending.
