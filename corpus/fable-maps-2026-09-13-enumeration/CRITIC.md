# Brief: independent critic for one ideonomic map (run 2026-09-13)
...
2. `corpus/fable-maps-2026-09-13-enumeration/BRIEF.md` — the standards the author was held to.
...
A fact about this seat: across the three previous fleets every one of the 38
critiques returned `admit-with-repairs`, never `admit`, never
`return-for-another-breath`, never `reject`. That distribution was set by the
menu and the seat, not by the maps. All four verdicts are live here: an
`admit` with zero repairs is a finding, and a `return-for-another-breath` is
cheaper for the atlas than a repaired map whose items are cases with no
mechanism. Do not find something because the seat expects it.

## Output

`corpus/fable-maps-2026-09-13-enumeration/<slug>.critique.json`:
