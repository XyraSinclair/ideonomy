-- | Gunkel's divisions of ideonomy, recovered from
-- ideonomy.mit.edu/division.html.
--
-- Each division is a binomen: THEME (the category of ideas treated) ->
-- FIELD (a name coined from Greek roots). Gunkel asserted 235 divisions
-- exist; 236 theme->binomen pairs are recovered here from the live page
-- (his count and the page's list do not quite reconcile). Each division
-- has its own theory, methods, organons, and ideas — i.e. each is a
-- fault-model (P3) for a whole region of thought, and the list itself is a
-- taxonomy of meta-level structural concepts.
module Ideonomy.Divisions (divisions, lookupDivision, lensPrompt) where

import Data.Char (toUpper)
import Ideonomy.Util (lower)

-- | The theme is matched case-insensitively; the page's themes are upper-case.
lookupDivision :: String -> Maybe String
lookupDivision theme = lookup (map toUpper theme) divisions
...
lensPrompt :: String -> String -> Either String String
lensPrompt theme subject = case lookupDivision theme of
  Nothing -> Left ("unknown division theme: " ++ show theme)
  Just field -> Right $
    "Apply " ++ field ++ " — the systematic study of " ++ t ++ " — to the subject "
    ++ "below. Enumerate the " ++ t ++ " present in or implied by it, type them, "
    ++ "and name the " ++ t ++ " that should exist but are conspicuously absent.\n\n"
    ++ "Subject: " ++ subject
  where
    t = lower theme
